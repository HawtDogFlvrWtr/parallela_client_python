...
config_log_level = default_config_values['DEFAULT']['log_level'].upper()
max_log_bytes = int(default_config_values['DEFAULT']['log_max_size_mb']) * 1_048_576
log_max_rotation_count = int(default_config_values['DEFAULT']['log_max_rotation_count'])
...
def return_usable_resource(config_value, available_value, to_bytes=False):
    if '%' in config_value: # Convert string to decimal
        max_value = available_value * (int(config_value.replace('%', '')) / 100)
        if to_bytes:
...
total_gpus = GPUDetector.get_gpu_counts()
...
for thread_num in range(usable_cpu_cores): # Create threads equal to the total number of cores from the config
    producer_thread = threading.Thread(target=thread_function, args=(q, thread_num,))
    producer_thread.start()

# Start requests thread
server_address = default_config_values['DEFAULT']['server_address']
callback_interval = default_config_values['DEFAULT']['server_callback_interval_secs']
ignore_server_cert = default_config_values['DEFAULT']['ignore_server_cert']

requests_thread = threading.Thread(target=callback_thread, args=(q, server_address, callback_interval, ignore_server_cert,))
requests_thread.start()

# if we exit, we need to join the queue
q.join()